...
class CardCipher:
    ...
    @staticmethod
    def type_from_cardid(cardid: str) -> int:
    ...
    def encode(cardid: str) -> str:
        ...
        groups[13] = 1
        groups[0] ^= CardCipher.type_from_cardid(cardid)

        for i in range(0, 14):
            groups[i] ^= groups[i - 1]

        # Scheme field is 1 for old-style, 2 for felica cards
        groups[14] = CardCipher.type_from_cardid(cardid)
    ...
    def decode(cardid: str) -> str:
        ...
        if groups[14] != CardCipher.type_from_cardid(finalvalue):
            raise CardCipherException(
                "Card type mismatch"
            )
        return finalvalue
